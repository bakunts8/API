...
public class UserTest extends BaseTest {
    ...
    public void updateUserTest() throws JsonProcessingException {
        ...
        int id = service.getUsersList().get(new Random().nextInt(service.getUsersList().size() - 1)).getId();
        UserData forUpdate = service.getUser(id);
        forUpdate.setEmail(fake.internet().emailAddress());

        UserData updated = service.updateUser("/users/" + id, forUpdate)
                .then()
                .statusCode(200)
                .extract()
                .as(UserData.class);
